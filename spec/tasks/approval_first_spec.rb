...
RSpec.describe 'scheduler namespace rake task' do
  ...
  it 'should change the approval_num for the resource to 2 if we are
      sending the first warning email' do
    run_approval_email
    expect(Resource.find_by_title('resource0').approval_num).to eq(2)
    expect(Resource.find_by_title('resource1').approval_num).to eq(2)
  end
end
