module ResourcesControllerUpload
    ...
    def mapRecord (row)
        return {
            'title': row['Title'],'url': row['Resource URL'],
            'resource_email': row['Email-General'],'resource_phone': row['Phone-General'],
            'address': row['Address'],
            'contact_email': row['Email-Contact'],'contact_name': row['Contact'],'contact_phone': row['Phone-Contact'],
            'description': row['Resource Description'],
            'types': row['Resource Type'],'audiences': row['Audience'],'population_focuses': row['Population Focus'], 'location': row['Location'],'campuses': row['Campuses'], 
            ...
            'notes': row['Notes'], 
            'approval_status': 0, 'approved_by': row['Approved by'],
            'updated_at': '', 'flagged': '', 'flagged_comment': ''
            }
    end
end
